...
int vmm_check_ret_code_sound = 0;
...
struct security_test_mem *g_test_vmm_alloc_sound;

//request vmm memory
struct security_test_msg *security_test_vmcall_sound(
...
struct security_test_mem *dma_debug_buffer_alloc_sound(
...
EXPORT_SYMBOL(dma_debug_buffer_alloc_sound);
...
static ssize_t vmm_check_res_show(struct device *dev,
			     struct device_attribute *attr,
			     char *buf)
{
	return sprintf(buf, "vmm_check_res=%d\n", vmm_check_ret_code_sound);
}
...
//relink bdl when the DMA engine is running
			      ...
			      const char *buf, size_t count)
{
	...
	case BDL_TO_NO_USED:
		...
		count = 1;
		break;
	case BDL_TO_LINUX_NOISE_MEMORY://'1'
		count = 1;
		if (g_test_noise) {
		...
		break;
	case BDL_TO_LINUX_ZERO_MEMORY://'2'
		count = 1;
		if (g_test_alloc) {
		...
		break;
	case BDL_TO_VMM_MEMORY://'3'
		count = 1;
		if (!g_test_vmm_alloc_sound)
			g_test_vmm_alloc_sound = dma_debug_buffer_alloc_sound(
				g_test_vmm_alloc_sound, MEM_TYPE_VMM, PAGE_4K_SIZE);

		if (g_test_vmm_alloc_sound)
			bdl_entry_hack_test(g_test_vmm_alloc_sound->phy_addr);

		msleep(1000);

		g_test_vmm_alloc_sound = security_test_alloc(
				CMD_CHECK_MEM, g_test_vmm_alloc_sound);
		pr_info("%s check-HVA=0x%lx, HPA=0x%lx\n ",
			__func__,
			g_test_vmm_alloc_sound->vir_addr,
			g_test_vmm_alloc_sound->phy_addr);

		g_test_vmm_alloc_sound = security_test_alloc(
				CMD_FREE_MEM, g_test_vmm_alloc_sound);
		pr_info("%s freed-HVA=0x%lx, HPA=0x%lx\n ",
			__func__,
			g_test_vmm_alloc_sound->vir_addr,
			g_test_vmm_alloc_sound->phy_addr);

		g_test_vmm_alloc_sound = NULL;
		break;

	case BDL_TO_LK_MEMORY://'4'
...
struct device_attribute dev_attr_vmm_check_res = {
	.attr	= { .name = "vmm_check_res", .mode = S_IRUGO },
	.show	= vmm_check_res_show,
};
#endif
