...
static struct xhci_dbc_sysfs_info dbc_info;
...
static u32 xhci_dbc_populate_strings(struct dbc_str_descs *strings)
{
	...
	utf8s_to_utf16s(dbc_info.string.serial,
	strlen(dbc_info.string.serial),
			UTF16_LITTLE_ENDIAN, (wchar_t *)s_desc->wData,
			DBC_MAX_STRING_LENGTH);

	s_desc->bLength   = (strlen(dbc_info.string.serial) + 1) * 2;
	...
	utf8s_to_utf16s(dbc_info.string.product,
	    strlen(dbc_info.string.product),
			UTF16_LITTLE_ENDIAN, (wchar_t *)s_desc->wData,
			DBC_MAX_STRING_LENGTH);

	s_desc->bLength   = (strlen(dbc_info.string.product) + 1) * 2;
	s_desc->bDescriptorType	= USB_DT_STRING;
	string_length		+= s_desc->bLength;
	string_length		<<= 8;
	...
	utf8s_to_utf16s(dbc_info.string.manufacturer,
		strlen(dbc_info.string.manufacturer),
			UTF16_LITTLE_ENDIAN, (wchar_t *)s_desc->wData,
			DBC_MAX_STRING_LENGTH);

	s_desc->bLength = (strlen(dbc_info.string.manufacturer) + 1) * 2;
	s_desc->bDescriptorType	= USB_DT_STRING;
	string_length		+= s_desc->bLength;
	string_length		<<= 8;
...
static void xhci_dbc_init_contexts(struct xhci_hcd *xhci, u32 string_length)
{
	...
	dev_info = cpu_to_le32((dbc_info.vid << 16) | dbc_info.protocol);
	writel(dev_info, &dbc->regs->devinfo1);

	dev_info = cpu_to_le32((dbc_info.device_rev << 16) | dbc_info.pid);
...
static ssize_t dbc_func_show(struct device *dev,
...
static ssize_t dbc_func_store(struct device *dev,
      struct device_attribute *attr,
...
static ssize_t function_show(struct device *dev,
     ...
     char *buf)
{
	return sprintf(buf, "%s\n", dbc_info.dbc_function);
}

static ssize_t function_store(struct device *dev,
      struct device_attribute *attr,
      const char *buf, size_t size)
{
	int ret = -EINVAL;

	if (size >= sizeof(dbc_info.dbc_function))
		ret = -EINVAL;
	else if (sscanf(buf, "%s",  dbc_info.dbc_function) == 1)
		ret = size;
...
static ssize_t manufacturer_show(struct device *dev,
     ...
     char *buf)
{
	return sprintf(buf, "%s\n", dbc_info.string.manufacturer);
}

static ssize_t manufacturer_store(struct device *dev,
      struct device_attribute *attr,
      const char *buf, size_t size)
{
	int ret = -EINVAL;

	if (size >= sizeof(dbc_info.string.manufacturer))
		ret = -EINVAL;
	else if (sscanf(buf, "%s",  dbc_info.string.manufacturer) == 1)
		ret = size;
...
static ssize_t product_show(struct device *dev,
     ...
     char *buf)
{
	return sprintf(buf, "%s\n", dbc_info.string.product);
}

static ssize_t product_store(struct device *dev,
      struct device_attribute *attr,
      const char *buf, size_t size)
{
	int ret = -EINVAL;

	if (size >= sizeof(dbc_info.string.product))
		ret = -EINVAL;
	else if (sscanf(buf, "%s", dbc_info.string.product) == 1)
		ret = size;
      ...
      const char *buf, size_t size)
{
	...
	if (size >= sizeof(dbc_info.string.serial))
		ret = -EINVAL;
	else if (sscanf(buf, "%s", dbc_info.string.serial) == 1)
		ret = size;
...
static ssize_t vid_store(struct device *dev,
      struct device_attribute *attr,
      const char *buf, size_t size)
{
	int value;
...
static ssize_t device_rev_show(struct device *dev,
     struct device_attribute *attr,
     char *buf)
{
	return sprintf(buf, "%d\n", dbc_info.device_rev);
}
static ssize_t device_rev_store(struct device *dev,
      struct device_attribute *attr,
      const char *buf, size_t size)
{
	int value;
			 ...
			 const char *buf, size_t count)
{
  ...
  if (!strncmp(buf, "enable", 6)) {
    if (dbc_info.func == DBC_FUNC_TTY) {
      ...
      ret = dbc_raw_register_device(xhci);
			xhci_info(xhci, "DbC now attached to /dev/dbc_raw\n");
    }
    if (ret < 0)
      return -EINVAL;
		xhci_dbc_start(xhci);
...
static DEVICE_ATTR_RW(function);
static DEVICE_ATTR_RW(manufacturer);
static DEVICE_ATTR_RW(product);
static DEVICE_ATTR_RW(serial);

static DEVICE_ATTR_RW(protocol);
static DEVICE_ATTR_RW(vid);
static DEVICE_ATTR_RW(pid);
static DEVICE_ATTR_RW(device_rev);
static DEVICE_ATTR_RW(dbc_func);
...
static void xhci_do_dbc_desc_init(void) {
 sscanf(DBC_STR_MANUFACTURER , "%s", dbc_info.string.manufacturer);
 sscanf(DBC_STR_PRODUCT , "%s", dbc_info.string.product);
 sscanf(DBC_STR_SERIAL , "%s", dbc_info.string.serial);

 dbc_info.protocol = DBC_PROTOCOL ;
 dbc_info.vid = DBC_VENDOR_ID;
 dbc_info.pid = DBC_PRODUCT_ID;
 dbc_info.device_rev = DBC_DEVICE_REV;
 dbc_info.func = DBC_FUNC_TTY ;
}
